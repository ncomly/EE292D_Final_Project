...
extension CVPixelBuffer {
  ...
  func centerThumbnail(ofSize size: CGSize ) -> CVPixelBuffer? {
    ...
    let inputImageRowBytes = CVPixelBufferGetBytesPerRow(self)
		print("inputImageRowBytes ", inputImageRowBytes)
    let imageChannels = 4

		let thumbnailSize = min(Int(Double(imageWidth) / 1.5), Int(Double(imageHeight) / 1.5))
...
//    }
		...
		let originX = imageWidth / 2 - thumbnailSize / 2
		let originY = imageHeight / 2 - thumbnailSize / 2
