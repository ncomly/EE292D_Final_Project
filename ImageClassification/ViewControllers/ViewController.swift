...
class ViewController: UIViewController {
  ...
  @IBOutlet weak var bottomSheetStateImageView: UIImageView!
	...
	private var isDoneRecording: Bool = false
	private var numFramesSampled: Int = 0
...
//	}
  ...
  private let expandThransitionThreshold: CGFloat = 40.0
	private let delayBetweenInferencesMs: Double = 5 // 34 // 1000

  // MARK: Instance Variables
  // Holds the results at any time
//  private var result: Result?
	private var cgImageCopy: Unmanaged<CGImage>?
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
		...
		let thumbnailSize = min(screenWidth/1.5, screenHeight/1.5)
		let xPos = screenWidth / 2 - thumbnailSize / 2
		let yPos = screenHeight / 2 - thumbnailSize / 2
		
		let rectHeight = thumbnailSize
		let rectWidth = thumbnailSize
...
extension ViewController: CameraFeedManagerDelegate {
  ...
  func didOutput(pixelBuffer: CVPixelBuffer) {
		// guard (isStartedRecording) else { return }
    let currentTimeMs = Date().timeIntervalSince1970 * 1000
    guard (currentTimeMs - previousInferenceTimeMs) >= delayBetweenInferencesMs else { return }
//		print("currentTime: ", currentTimeMs)
//		print("previousTime: ", previousInferenceTimeMs)
		print("diff: ", currentTimeMs - previousInferenceTimeMs)
    previousInferenceTimeMs = currentTimeMs
    
		// guard (isDoneRecording) else { return }
		
//		print("Passing new frame to model")
		
    // Pass the pixel buffer to TensorFlow Lite to perform inference.
//		var cgImage: Unmanaged<CGImage>?
//		if (modelDataHandler?.runModel(onFrame: pixelBuffer) != nil){
//			(result, cgImage) = (modelDataHandler?.runModel(onFrame: pixelBuffer))!
//		}
		guard
			let (result, cgImage) = modelDataHandler?.runModel(onFrame: pixelBuffer)
		else {return}
		
		if cgImage != nil {print("cgImage not nil in didOutput")}
		self.cgImageCopy = cgImage
		
		if self.cgImageCopy != nil {
			print("cgImage not nil in viewer")
//			self.view.reloadInputViews()
//			DispatchQueue.main.async {
//				let imageView = UIImageView(image: UIImage(cgImage: self.cgImageCopy!.takeRetainedValue()))
//				self.view.addSubview(imageView)
//			}
		}
